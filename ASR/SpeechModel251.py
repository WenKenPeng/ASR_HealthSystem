		...
		try:
			...
			for i in range(data_count):
				...
				if(edit_distance <= words_n): # 当编辑距离小于等于句子字数时
					...
					word_error_num += words_n # 直接加句子本来的总字数
			...
			#print('*[测试结果] 语音识别 ' + str_dataset + ' 集语音单字正确率：', word_error_num / words_num * 100, '%')
			print('*[Test Result] Speech Recognition ' + str_dataset + ' set word error ratio: ', word_error_num / words_num * 100, '%')
			if(out_report == True):
				txt += '*[测试结果] 语音识别 ' + str_dataset + ' 集语音单字正确率： ' + str(word_error_num / words_num * 100) + ' %'
				txt_obj.write(txt)
				txt = ''
				txt_obj.close()
...
if(__name__=='__main__'):
	...
	ms.TrainModel(datapath, epoch = 50, batch_size = 16, save_step = 625000)
